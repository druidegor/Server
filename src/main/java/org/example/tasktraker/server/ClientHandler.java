...
public class ClientHandler implements Runnable {
    ...
    private Response processRequest(Request request) {
        switch (request.getCommand()) {
            ...
            case "GENERATE_PROJECT_REPORT":
                return handleGenerateReport(request.getPayload());
            case "AUTO_ASSIGN_TASK":
                return handleAutoAssignTask(request.getPayload());
            default:
    ...
    private Response handleGenerateReport(Object payload) {
        try {
            int projectId = (int) payload;
            String report = taskService.generateProjectReport(projectId);
            return ResponseFactory.createSuccess("Отчет сгенерирован", report);
        } catch (Exception e) {
            return ResponseFactory.createError(e.getMessage());
        }
    }

    private Response handleAutoAssignTask(Object payload) {
        try {
            int[] data = (int[]) payload; // Ожидаем массив [taskId, projectId]
            int taskId = data[0];
            int projectId = data[1];
            taskService.autoAssignTask(taskId, projectId);
            return ResponseFactory.createSuccess("Задача автоматически назначена на самого свободного разработчика");
        } catch (Exception e) {
            return ResponseFactory.createError(e.getMessage());
        }
    }
